...
/** Isolated-lab executable. Test-owned credentials; no provider permit or native packet-budget claim. */
public final class NativeDiagnosticRole {
    ...
    NativeDiagnosticRole(Path file) throws Exception {
        ...
        require(config.get("version").equals("1") && config.get("mode").equals("first-contact"));
    ...
    void finish(PeerConnection peer) throws Exception {
        ...
        System.out.println("{\"event\":\"transport_complete\",\"role\":\"" + config.get("role") + "\",\"independentRoundTrips\":2,"
            ...
            + (bind.getAddress().length == 4 ? 4 : 6) + ",\"admission\":\"test-owned\",\"gameplay\":false}");
    }
    ...
    void client() throws Exception {
        ...
        try {
            ...
            peer.onStateChange.register((p, state) -> { if (state == PeerState.RTC_FAILED) failure.compareAndSet(null, new IllegalStateException("Transport failed")); });
            ...
            for (String line : peer.localDescription().split("\r?\n"))
                if (!line.startsWith("a=candidate:") && !line.equals("a=end-of-candidates") && !line.isEmpty()) offer.append(line).append("\r\n");
            offer.append("a=candidate:1 1 UDP 2130706431 ").append(config.get("publicAddress")).append(' ').append(config.get("publicPort"))
                .append(" typ host\r\na=end-of-candidates\r\n");
            writeFile("offerPath", offer.toString());
            ...
            finish(peer);
        } finally { require(peer.closeAndAwait(Duration.ofSeconds(5))); }
    }
        ...
        try (IceUdpMuxListener listener = new IceUdpMuxListener(bind, localPort, 1, Duration.ofSeconds(5), Runnable::run, request -> {
            ...
            return CompletableFuture.completedFuture(IceUdpMuxListener.Acceptance.builder(offer.get(), config.get("localPassword"))
                ...
                .initialize(created -> {
                    ...
                    created.onStateChange.register((p, state) -> { if (state == PeerState.RTC_FAILED) failure.compareAndSet(null, new IllegalStateException("Transport failed")); });
                }).build());
        })) {
            ...
            require(peer.closeAndAwait(Duration.ofSeconds(5))); peer = null;
            require(listener.statistics().agents() == 0 && listener.statistics().mappedTuples() == 0 && listener.statistics().pendingRequests() == 0);
        } finally { if (peer != null) require(peer.closeAndAwait(Duration.ofSeconds(5))); }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("{\"event\":\"failed\",\"reason\":\"diagnostic_not_completed\",\"errorType\":\"" + error.getClass().getSimpleName()
                ...
                + ",\"lastNotifications\":" + (role == null ? 0 : role.lastNotifications) + "}"); exit = 1;
        }
        System.exit(exit);
    }
}
